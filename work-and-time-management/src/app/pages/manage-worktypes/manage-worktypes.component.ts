...
export class ManageWorktypesComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        this.manageService.routeEvents(
            ...
            RouterRoutesEnum.EDIT_WORKTYPE
        );
    ...
    setupModal(worktype: WorkType, routeResponse: Params) {
        if (!worktype) {
            const fetchByIdSubs = this.worktypeService
                .fetchWorktypeById(routeResponse.id)
                .subscribe(res2 => {
                    if (!res2 || !res2.data) {
    ...
    edit(worktype: WorkType, index: number) {
        ...
        this.router.navigate([
            `${RouterRoutesEnum.EDIT_WORKTYPE}/${worktype.id}`,
        ]);
    }
    ...
    postNewWorktype(formValues: Partial<WorkType>) {
        ...
        this.worktypeService
            .postNewWorktype(newWorktype)
            .subscribe(worktype => {
                this.worktypeService.addNewWorktypeToStore(
                    newWorktype,
                    worktype.id
                );
    ...
    updateWorktype(formValues: Partial<WorkType>, activeWorktype: WorkType) {
        ...
        this.worktypeService
            .putWorktype(activeWorktype.id, updatedWorktype)
            .subscribe(() => {
                this.worktypeService.updateWorktype(
                    activeWorktype,
                    updatedWorktype
                );
